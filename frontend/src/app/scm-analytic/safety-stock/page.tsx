...
export default function SafetyStockPage() {
  ...
  const handleSaveToGlobal = async () => {
    ...
    const { error } = await supabase.from('processed_results').insert([{ module: 'safety_stock', result_json: JSON.stringify({ compressed: true, data: LZString.compressToBase64(JSON.stringify(dataCopy)) }) }]);
    if (error) {
  ...
  return (
    <div id="export-container" className="space-y-8 max-w-[1550px] mx-auto pb-16 animate-in fade-in duration-500 text-foreground">
      ...
      <PageHeader
        icon={ShieldCheck}
        eyebrow="SCM Analytic • Inventory Protection"
        title="Dynamic"
        highlight="Safety Stock & ROP"
        description="Kalkulasi cadangan pengaman (Safety Stock) dan titik pemesanan ulang (Reorder Point) secara dinamis memadukan variabilitas demand dan fluktuasi lead time pengiriman per SKU per Cabang."
        actions={
          <>
      ...
      {showHowTo && (
        <GlassCard className="no-export p-6 border-indigo-500/30 bg-card backdrop-blur-xl animate-fade-in">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-sm text-muted-foreground">
            ...
            <div className="space-y-3">
              <h4 className="font-semibold text-foreground">⚙️ Z-Score & Net Flow Position:</h4>
      ...
      <div className="no-export space-y-3">
        <div className="flex items-center justify-between">
          ...
          <span className="text-xs text-muted-foreground italic hidden sm:inline">Klik tab untuk memproyeksikan lonjakan permintaan atau keterlambatan suplai!</span>
        </div>
      ...
      <GlassCard className="no-export p-4 bg-card border-border flex flex-col sm:flex-row items-center justify-between gap-4">
              ...
              <div className="h-[400px]">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={zoneChartData} margin={{ top: 10, right: 20, left: 0, bottom: 5 }}>
                    ...
                    <Tooltip contentStyle={{ backgroundColor: 'hsl(var(--popover))', borderColor: 'hsl(var(--border))', color: 'hsl(var(--popover-foreground))' }} />
                    <Legend />
                    <Bar dataKey="Red Zone" stackId="zones" fill="hsl(var(--destructive))" radius={[0, 0, 0, 0]} />
                    <Bar dataKey="Yellow Zone" stackId="zones" fill="hsl(var(--chart-4))" radius={[0, 0, 0, 0]} />
                    <Bar dataKey="Green Zone" stackId="zones" fill="hsl(var(--accent))" radius={[4, 4, 0, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </GlassCard>
                  ...
                  <BarChart data={ltChartData} layout="vertical" margin={{ top: 10, right: 20, left: 80, bottom: 5 }}>
                    ...
                    <Bar dataKey="avg_lead_time" name="Avg Lead Time" radius={[0, 4, 4, 0]}>
                      {ltChartData.map((entry: any, idx: number) => (
                        <Cell key={idx} fill={entry.avg_lead_time > 20 ? 'hsl(var(--destructive))' : entry.avg_lead_time > 12 ? 'hsl(var(--chart-4))' : 'hsl(var(--accent))'} />
                      ))}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
