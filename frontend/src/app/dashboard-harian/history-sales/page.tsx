...
import {
  ...
  Filter, Search, X, RefreshCw, CheckCircle2, Info, ArrowUpRight, ArrowDownRight, ShieldAlert, Zap, Cloud } from 'lucide-react';
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-2 pt-2 border-t border-slate-200">
            <button
              ...
              className="flex-1 py-2 px-3 rounded-xl bg-muted hover:bg-primary font-bold text-xs text-muted-foreground hover:text-primary-foreground transition"
            >
              Reset
            </button>
            <button
...
export default function HistorySalesPage() {
  ...
  const handleSaveToGlobal = async () => {
    ...
    const dataCopy = { ...parsed, processed_at: timestamp };
        const { error } = await supabase.from('processed_results').insert([{ module: 'history_sales', result_json: JSON.stringify({ compressed: true, data: LZString.compressToBase64(JSON.stringify(dataCopy)) }) }]);
    if (error) {
